...
import com.creativetrends.tungsten.adapters.AppInfoAdapter;
import com.creativetrends.tungsten.dialogs.BackupRestoreDialogFragment;
import com.creativetrends.tungsten.dialogs.ShareDialogFragment;
import com.creativetrends.tungsten.schedules.Scheduler;
import com.creativetrends.tungsten.ui.HandleMessages;
import com.creativetrends.tungsten.ui.Help;
import com.creativetrends.tungsten.ui.LanguageHelper;
import com.creativetrends.tungsten.ui.NotificationHelper;

public class TungstenBackup extends BaseActivity
implements SharedPreferences.OnSharedPreferenceChangeListener
{
    public static final String TAG = TungstenBackup.class.getSimpleName().toLowerCase();
    ...
    public void onResume()
    {
        ...
        {
            Utils.reloadWithParentStack(TungstenBackup.this);
    ...
    public void callBackup(final AppInfo appInfo, final int backupMode)
    {
        Thread backupThread = new Thread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                if(backupDir != null)
                {
                    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(TungstenBackup.this);
                    Crypto crypto = null;
                    if(prefs.getBoolean("enableCrypto", false) && Crypto.isAvailable(TungstenBackup.this))
                        crypto = getCrypto();
                    backupRet = BackupRestoreHelper.backup(TungstenBackup.this, backupDir, appInfo, shellCommands, backupMode);
                    if(backupRet == 0 && crypto != null)
                    {
                        crypto.encryptFromAppInfo(TungstenBackup.this, backupDir, appInfo, backupMode, prefs);
                ...
                if(backupRet == 0)
                {
                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.backupSuccess), appInfo.getLabel(), true);
                }
                else
                {
                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.backupFailure), appInfo.getLabel(), true);
                    Utils.showErrors(TungstenBackup.this);
                }
            }
        });
        backupThread.start();
        ...
        Thread restoreThread = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                if(backupDir != null)
                {
                    ...
                    Crypto crypto = null;
                    if(Crypto.isAvailable(TungstenBackup.this) && Crypto.needToDecrypt(backupDir, appInfo, mode))
                        crypto = getCrypto();
                    ret = BackupRestoreHelper.restore(TungstenBackup.this, backupDir, appInfo, shellCommands, mode, crypto);
                    refresh();
                }
                handleMessages.endMessage();
                if(ret == 0)
                {
                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.restoreSuccess), appInfo.getLabel(), true);
                }
                else
                {
                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.restoreFailure), appInfo.getLabel(), true);
                    Utils.showErrors(TungstenBackup.this);
                }
            }
        });
    ...
    public void refresh()
    {
        Thread refreshThread = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                appInfoList = AppInfoHelper.getPackageInfo(TungstenBackup.this, backupDir, true);
                runOnUiThread(new Runnable()
                {
                    public void run()
                    {
    ...
    public boolean onCreateOptionsMenu(Menu menu)
    {
        ...
        if(android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB)
        {
            ...
            search.setOnQueryTextListener(new OnQueryTextListener()
            {
                ...
                {
                    if(TungstenBackup.this.adapter != null)
                        TungstenBackup.this.adapter.getFilter().filter(newText);
                    return true;
                }
                @Override
                ...
                {
                    if(TungstenBackup.this.adapter != null)
                        TungstenBackup.this.adapter.getFilter().filter(query);
                    return true;
                }
            });
                    ...
                    public void onClick(DialogInterface dialog, int which)
                    {
                        Thread uninstallThread = new Thread(new Runnable()
                        {
                            public void run()
                            {
                                ...
                                if(ret == 0)
                                {
                                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.uninstallSuccess), appInfo.getLabel(), true);
                                }
                                else
                                {
                                    NotificationHelper.showNotification(TungstenBackup.this, TungstenBackup.class, notificationId++, getString(R.string.uninstallFailure), appInfo.getLabel(), true);
                                    Utils.showErrors(TungstenBackup.this);
                                }
                            }
                        });
    ...
    public void onSharedPreferenceChanged(SharedPreferences preferences, String key)
    {
        if(key.equals("pathBackupFolder"))
        {
            ...
            backupDir = Utils.createBackupDir(TungstenBackup.this, backupDirPath);
            refresh();
        }
        ...
        if(android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.HONEYCOMB)
        {
            ...
            if(child.getClass() != android.widget.EditText.class)
            {
                ...
                et.addTextChangedListener(new android.text.TextWatcher()
                {
                    ...
                    {
                        TungstenBackup.this.adapter.getFilter().filter(s.toString());
                    }
                });
    ...
    private class InitRunnable implements Runnable
    {
        ...
        {
            SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(TungstenBackup.this);
            prefs.registerOnSharedPreferenceChangeListener(TungstenBackup.this);
            shellCommands = new ShellCommands(prefs, users);
            String langCode = prefs.getString("languages", "system");
            LanguageHelper.initLanguage(TungstenBackup.this, langCode);
            ...
            if(!checked)
            {
                ...
                LanguageHelper.legacyKeepLanguage(TungstenBackup.this, langCode);
                if(!haveSu)
                {
                    Utils.showWarning(TungstenBackup.this, "", getString(R.string.noSu));
                }
                checkBusybox();
                handleMessages.endMessage();
            }
            ...
            adapter = new AppInfoAdapter(TungstenBackup.this, R.layout.listlayout, appInfoList);
